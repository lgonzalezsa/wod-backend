...
# Origin: https://raw.githubusercontent.com/docker-library/docker/master/dockerd-entrypoint.sh
# Modify sheel to /bin/bash and add the above comment
...
if [ "$#" -eq 0 ] || [ "${1#-}" != "$1" ]; then
	...
	; then
		...
		set -- dockerd \
			...
			--bip=192.168.70.1/24 \
			--default-address-pool base=10.64.0.0/16,size=24 \
			--tlsverify \
